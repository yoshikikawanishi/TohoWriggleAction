...
public class ConfigButton : MonoBehaviour {
    ...
    //ショットボタン
    public void Shot_Button() {
        if (!wait_Input && InputManager.Instance.GetKeyDown(MBLDefine.Key.Jump)) {
            StartCoroutine(Change_Key_Config(MBLDefine.Key.Shot, shot_Button));
        }
    }
    ...
    public void Fly_Button() {
        if (!wait_Input && InputManager.Instance.GetKeyDown(MBLDefine.Key.Jump)) {
            StartCoroutine(Change_Key_Config(MBLDefine.Key.Fly, fly_Button));
        }
    }
    ...
    public void Pause_Button() {
        if (!wait_Input && InputManager.Instance.GetKeyDown(MBLDefine.Key.Jump)) {
            StartCoroutine(Change_Key_Config(MBLDefine.Key.Pause, pause_Button));
        }
    }
